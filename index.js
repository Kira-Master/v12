...
Lexxy.sendMessage(from, gfx, image, { quoted: mek, caption: '*Logo By Kira - Master*'})
break
case 'gfx3':
case 'gfx4':
...
} catch (e) { reply("ERROR! TRY AGAIN.\nIF HAPPEN AGAIN, CALL OWNER!")}
break	
...
"listMessage": {
"title": '「 STORE MENU 」',
"description": `*Created By Kira Master*`,
"buttonText": "𝙋𝙄𝙇𝙄𝙃 𝘿𝙄𝙎𝙄𝙉𝙄",
"listType": "SINGLE_SELECT",
"sections": [
{
...
Lexxy.sendMessage(from, bf, image, { quoted: flexx, caption: 'Logo By Kira - Master' })
break
case 'kaneki':
case 'rem':
case 'lolimaker':
...
case 'cekidml':
if (isBanned)return sticBanned(from)
if (args.length ==0)return reply('ID ML Nya Mana Kak?')
id = args.join(" ")
res = await fetchJson(`https://pingiping.com/api/searching/mlcek?UserId=${id}`)
reply(`*Nick Mobile Legends* : *${res.nickname}*`)
break
case 'cekidff':
if (isBanned)return sticBanned(from)
if (args.length ==0)return reply('ID FF Nya Mana Kak?')
id = args.join(" ")
res = await fetchJson(`https://pingiping.com/api/searching/ffcek?UserId=${id}`)
reply(`*Nick FreeFire* : *${res.nickname}*`)
break
case 'herodetail':
...
fakethumb(buffer,'By Kira - Master')
fs.unlinkSync(ran)
})
break
case 'searchgrup':
...
Lexxy.sendMessage('6282322153431@s.whatsapp.net', options, text, {quoted: flexx})
Lexxy.sendMessage('6282322153431@s.whatsapp.net', options, text, {quoted: flexx})
